...
public enum AstCode {
    ...
    public static boolean expandMacro(final StrongBox<AstCode> code, final StrongBox<Object> operand, final MethodBody body, final int offset) {
        ...
        switch (op) {
            ...
            case __IStore0:
                code.set(Store);
                operand.set(body.getVariables().tryFind(0, offset + 1));
                return true;
            case __IStore1:
                code.set(Store);
                operand.set(body.getVariables().tryFind(1, offset + 1));
                return true;
            case __IStore2:
                code.set(Store);
                operand.set(body.getVariables().tryFind(2, offset + 1));
                return true;
            case __IStore3:
                code.set(Store);
                operand.set(body.getVariables().tryFind(3, offset + 1));
                return true;
            case __LStore0:
                code.set(Store);
                operand.set(body.getVariables().tryFind(0, offset + 1));
